...
//        }
    ...
    public static void openFile(FileHolder fileholder, Context c) {
        ...
        Toast.makeText(c, "123456789", Toast.LENGTH_SHORT).show();

        final CharSequence[] charSequences = {"以\"XINHAO_HAN\"文本打开[针对没有root用户]", "其他方式打开(没有root的用户,几乎用不了)"};


        AlertDialog.Builder ab = new AlertDialog.Builder(c);
        ab.setItems(charSequences, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                ...
                switch (which) {
                    ...
                    case 0:
                        TermuxData.getInstall().fileUrl = fileholder.getFile().getAbsolutePath();
                        c.startActivity(new Intent(c, EditTextActivity.class));
