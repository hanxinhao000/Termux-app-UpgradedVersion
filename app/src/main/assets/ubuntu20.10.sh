...
if [ "$first" != 1 ];then
    if [ ! -f "ubuntu.tar.gz" ]; then
        ...
        if [ "$(dpkg --print-architecture)" = "aarch64" ];then
            wget http://cdimage.ubuntu.com/ubuntu-base/releases/20.10/beta/ubuntu-base-20.10-beta-base-arm64.tar.gz -O ubuntu.tar.gz
        elif [ "$(dpkg --print-architecture)" = "arm" ];then
            wget http://cdimage.ubuntu.com/ubuntu-base/releases/20.10/beta/ubuntu-base-20.10-beta-base-armhf.tar.gz -O ubuntu.tar.gz
        elif [ "$(dpkg --print-architecture)" = "x86_64" ];then
            wget http://cdimage.ubuntu.com/ubuntu-base/releases/20.10/beta/ubuntu-base-20.10-beta-base-amd64.tar.gz -O ubuntu.tar.gz
        elif [ "$(dpkg --print-architecture)" = "i*86" ];then
            wget http://cdimage.ubuntu.com/ubuntu-base/releases/20.10/beta/ubuntu-base-20.04-base-i386.tar.gz -O ubuntu.tar.gz
