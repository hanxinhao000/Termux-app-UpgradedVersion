...
if [ "$first" != 1 ];then
    if [ ! -f "debian.tar.xz" ]; then
        ...
        if [ "$(dpkg --print-architecture)" = "aarch64" ];then
            wget http://127.0.0.1:19956/user/debian_arm64_rootfs.tar.xz -O debian.tar.xz -q
        elif [ "$(dpkg --print-architecture)" = "arm" ];then
            wget http://127.0.0.1:19956/user/debian_armhf_rootfs.tar.xz -O debian.tar.xz -q
        elif [ "$(dpkg --print-architecture)" = "x86_64" ];then
            wget http://127.0.0.1:19956/user/debian_amd_rootfs.tar.xz -O debian.tar.xz -q
        elif [ "$(dpkg --print-architecture)" = "x86" ];then
            wget http://127.0.0.1:19956/user/debian_i386_rootfs.tar.xz -O debian.tar.xz -q
