...
if [ "$first" != 1 ];then
    if [ ! -f "alpine.tar.xz" ]; then
        ...
        if [ "$(dpkg --print-architecture)" = "aarch64" ];then
            wget http://127.0.0.1:19956/user/alpine_arm64_rootfs.tar.xz -O alpine.tar.xz -q
        elif [ "$(dpkg --print-architecture)" = "arm" ];then
            wget http://127.0.0.1:19956/user/alpine_armhf_rootfs.tar.xz -O alpine.tar.xz -q
        elif [ "$(dpkg --print-architecture)" = "x86_64" ];then
            wget http://127.0.0.1:19956/user/alpine_amd_rootfs.tar.xz -O alpine.tar.xz -q
        elif [ "$(dpkg --print-architecture)" = "x86" ];then
            wget http://127.0.0.1:19956/user/alpine_i386_rootfs.tar.xz -O alpine.tar.xz -q
