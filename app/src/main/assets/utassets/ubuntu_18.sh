...
if [ "$first" != 1 ];then
    if [ ! -f "ubuntu.tar.gz" ]; then
        ...
        if [ "$(dpkg --print-architecture)" = "aarch64" ];then
            wget http://127.0.0.1:19956/user/ubuntu-base-18.04.5-base-arm64.tar.gz -O ubuntu.tar.gz -q
        elif [ "$(dpkg --print-architecture)" = "arm" ];then
            wget http://127.0.0.1:19956/user/ubuntu-base-18.04.5-base-armhf.tar.gz -O ubuntu.tar.gz -q
        elif [ "$(dpkg --print-architecture)" = "x86_64" ];then
            wget http://127.0.0.1:19956/user/ubuntu-base-18.04.5-base-amd64.tar.gz -O ubuntu.tar.gz -q
        elif [ "$(dpkg --print-architecture)" = "x86" ];then
            wget http://127.0.0.1:19956/user/ubuntu-base-18.04.5-base-i386.tar.gz -O ubuntu.tar.gz -q
