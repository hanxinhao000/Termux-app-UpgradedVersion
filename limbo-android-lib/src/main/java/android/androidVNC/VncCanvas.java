...
public class VncCanvas extends AppCompatImageView {
	...
	void initializeVncCanvas(ConnectionBean bean, final Runnable setModes) {
		...
		final ProgressDialog pd = ProgressDialog.show(getContext(), "连接到VM控制台", "请等待...", true,
				true, new DialogInterface.OnCancelListener() {
					...
					public void onCancel(DialogInterface dialog) {
						...
						handler.post(new Runnable() {
							public void run() {
								Utils.showErrorMessage(getContext(), "VNC连接失败!");
							}
						});
					}
				});
		...
		Thread t = new Thread() {
			...
			public void run() {
				try {
					...
					width = size.x;
					height = size.y;
                    Log.e("XINHAO_HAN_EX", "VNC密码外: " + connection.getPassword() );
                    ...
                    LimboApplication.mHandlerY.post(new Runnable() {
                        @Override
                        public void run() {

                            Toast.makeText(getContext(), "连接失败,请检查填入信息是否正确 !", Toast.LENGTH_SHORT).show();
                        }
                    });
	...
	void connectAndAuthenticate(String us, String pw) throws Exception {
		Log.i(TAG, "连接到: " + connection.getAddress() + ", 端口号: " + connection.getPort() + "...");
		Log.e("XINHAO_HAN", "连接到: " + connection.getAddress() + ", 端口号: " + connection.getPort() + "...");

		rfb = new RfbProto(connection.getAddress(), connection.getPort());
		if (LOCAL_LOGV) {
			Log.v(TAG, "连接到服务器");
			Log.e("XINHAO_HAN", "连接到服务器");

		}
        Log.e("XINHAO_HAN", "RFB服务器支持协议版本前 " + rfb.serverMajor + "." + rfb.serverMinor);
		rfb.readVersionMsg();
        Log.e("XINHAO_HAN","RFB服务器支持协议版本后 " + rfb.serverMajor + "." + rfb.serverMinor);

		rfb.writeVersionMsg();
		Log.i(TAG, "使用RFB协议版本 " + rfb.clientMajor + "." + rfb.clientMinor);
