...
//                        panner.stop();
	...
	public void onCreate(Bundle icicle) {
        ...
        String stringExtra = getIntent().getStringExtra("vncPassword");
        ...
        if(stringExtra == null || stringExtra.isEmpty()){
            ...
            String password = getIntent().getStringExtra("passwordD1");


            Log.e("XINHAO_HAN", "VNC密码: " + password );
            Log.e("XINHAO_HAN", "VNC端口: " + port );
            Log.e("XINHAO_HAN", "VNC地址: " + adress );
            connection.setPassword(password);
            connection.setPort(Integer.parseInt(port));
            connection.setAddress(adress);


        }else{
            Toast.makeText(this, "有密码启动", Toast.LENGTH_SHORT).show();
            connection.setPassword(stringExtra);
        }
	...
	class DPadPanTouchMouseMode implements AbstractInputHandler {
		...
		public boolean onTouchEvent(MotionEvent event) {
			// Mouse Pointer Control Mode
			// Pointer event is absolute coordinates.Context.INPUT_METHOD_SERVICE
