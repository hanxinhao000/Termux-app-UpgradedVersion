...
public class UIUtils {
    ...
    /*    if(LimboActivity.vmexecutor == null){
            try {
                LimboActivity.vmexecutor = new VMExecutor(LimboActivity.currMachine, LimboApplication.limboApplication);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }*/
        ...
        Log.e(TAG, "onKeyboard: " + "键盘标记");

        if (paused == 1)
            return !toggle;
/*        InputMethodManager  mInputMethodManager = ((InputMethodManager) activity
            .getSystemService(Context.INPUT_METHOD_SERVICE));
        mInputMethodManager.showSoftInput(activity.getCurrentFocus(), 0);*/
